...
const calcDisplayBalance = function (account) {
  ...
  labelBalance.textContent = formattedCur(account.balance, account.locale, account.currency);
};
...
const calcDisplaySummary = function (account) {
  ...
  labelSumIn.textContent = formattedCur(incomes, account.locale, account.currency);;

  const out = account.movements.filter(mov => mov < 0).reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = formattedCur(Math.abs(out), account.locale, account.currency);;
  ...
  labelSumInterest.textContent = formattedCur(interest, account.locale, account.currency);;
};
...
btnTransfer.addEventListener('click', function (event) {
  ...
  if (amount > 0 && currentAccount.balance >= amount && receiverAccount && currentAccount?.username !== receiverAccount.username) {
    // Doing the transfer
...
btnClose.addEventListener('click', function (event) {
  ...
  if (currentAccount.username === inputCloseUsername.value && currentAccount.pin === +inputClosePin.value) {
    ...
    // Hide UI and Reset Welcome phrase
    labelWelcome.textContent = 'Log in to get started';
    containerApp.style.opacity = 0;
